...
export default function CategoryForm() {
    ...
    return <form  onChange={ changeHandler }onSubmit={ submitHandler } className="max-w-lg" > 
        ...
        <input type="text" name="title" id="title" value={form.title} onChange={ changeHandler } className="outline-none block w-[300px] p-1.5 border border-gray-300 rounded-md mb-7.5" /> 

        <label htmlFor="slug" className="block text-sm mb-2.5" > اسلاگ </label> 
        <input type="text" name="slug" id="slug" value={form.slug} onChange={ changeHandler } className="outline-none block w-[300px] p-1.5 border border-gray-300 rounded-md mb-7.5" /> 

        <label htmlFor="icon" className="block text-sm mb-2.5" > آیکون </label> 
        <input type="text" name="icon" id="icon" value={form.icon} onChange={ changeHandler } className="outline-none block w-[300px] p-1.5 border border-gray-300 rounded-md mb-7.5" /> 
